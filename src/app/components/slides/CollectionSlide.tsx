...
function AreaRow({ areas, accentColor }: { areas: AreaDetail[]; accentColor: string }) {
  return (
    <div className="collection-area-rows flex flex-col gap-0.5 sm:gap-2 lg:gap-1 w-full">
      {areas.map((a) => (
        <div
          key={a.label}
          className={`collection-area-cell flex items-center gap-1 sm:gap-2 lg:gap-2 rounded-lg px-1 py-0.5 sm:px-2.5 sm:py-1.5 lg:px-2.5 lg:py-1.5 ${
...
export function CollectionSlide({ total }: { total: number }) {
  ...
  return (
    <SlideLayout bgImage={collectionBg} overlay="darker" slideNumber={6} totalSlides={total}>
      <div className="collection-slide-compact w-full max-w-6xl mx-auto flex flex-col items-center text-center gap-2 sm:gap-4 md:gap-5 lg:gap-2">
        ...
        <motion.p
          ...
          className="collection-body text-white/60 max-w-lg lg-lock-22 text-[11px]"
        >
          {col.body}
        </motion.p>

        <div className="collection-grid mt-1 grid w-full grid-cols-2 items-stretch gap-1.5 sm:mt-4 sm:gap-4 lg:mt-2 lg:grid-cols-3 lg:gap-4">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.6 }}
            className="collection-card collection-card--prime flex h-full min-h-0 flex-col items-center rounded-xl border border-[#C9A96E]/20 bg-white/5 p-2 backdrop-blur-sm sm:p-5 lg:p-4"
          >
            <div className="flex w-full flex-col items-center gap-1 sm:gap-2 lg:gap-1.5">
              <div className="collection-icon-wrap flex h-6 w-6 items-center justify-center rounded-full border border-[#C9A96E]/30 bg-[#C9A96E]/10 sm:h-12 sm:w-12 lg:h-8 lg:w-8">
          ...
          <motion.div
            ...
            className="collection-card collection-card--premium flex h-full min-h-0 flex-col items-center rounded-xl border border-[#C9A96E]/30 bg-[#C9A96E]/5 p-2 backdrop-blur-sm sm:p-5 lg:p-4"
          >
            <div className="collection-icon-wrap flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-[#C9A96E]/40 bg-[#C9A96E]/15 sm:h-12 sm:w-12 lg:h-8 lg:w-8">
          ...
          <motion.div
            ...
            className="collection-card collection-card--included col-span-2 lg:col-span-1 bg-white/[0.03] backdrop-blur-sm border border-white/10 rounded-xl p-2 sm:p-5 lg:p-4 flex flex-col gap-1 sm:gap-3 lg:gap-2"
          >
              ...
              {col.furniture.map((label, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="collection-furniture-item flex items-center gap-1 sm:gap-2 lg:gap-2 bg-white/[0.04] hover:bg-white/[0.07] transition-colors rounded-lg px-1.5 py-1 sm:px-2.5 sm:py-1.5 lg:px-2.5 lg:py-1.5"
                  >
                    <div className="collection-furniture-icon w-4 h-4 sm:w-5 sm:h-5 lg:w-5 lg:h-5 rounded bg-[#C9A96E]/8 flex items-center justify-center shrink-0">
